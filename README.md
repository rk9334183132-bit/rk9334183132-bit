<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0f0c29,50:302b63,100:24243e&height=200&section=header&text=Rakesh%20Kumar&fontSize=60&fontColor=c792ea&fontAlignY=38&desc=Digital%20Systems%20%26%20RTL%20Design%20Engineer%20%E2%80%A2%20EE%20%40%20NIT%20Rourkela&descSize=16&descAlignY=60&descColor=a9b1d6" />

[![Typing SVG](https://readme-typing-svg.demolab.com?font=Fira+Code&size=18&pause=1000&color=7AA2F7&center=true&vCenter=true&width=700&lines=RTL+Design+%7C+FPGA+%7C+VLSI+Systems;Verilog+%7C+SystemVerilog+%7C+UART+%7C+SPI+%7C+AXI;Logic+Verification+%26+Testbench+Design;Building+synthesizable+digital+hardware+%F0%9F%94%8C)](https://git.io/typing-svg)

<p>
  <img src="https://img.shields.io/badge/STATUS-OPEN%20TO%20COLLABORATE-brightgreen?style=for-the-badge&logo=github&logoColor=white" />
  <img src="https://img.shields.io/badge/%F0%9F%93%8D%20LOCATION-ROURKELA,%20INDIA-7aa2f7?style=for-the-badge" />
  <img src="https://img.shields.io/badge/%F0%9F%8E%AF%20FOCUS-DIGITAL%20SYSTEMS%20%26%20VLSI-c792ea?style=for-the-badge" />
</p>

</div>

---

## `whoami`

```python
class DigitalSystemsEngineer:
    def __init__(self):
        self.name        = "Rakesh Kumar"
        self.institution = "National Institute of Technology, Rourkela"
        self.degree      = "B.Tech in Electrical Engineering"
        self.domain      = "RTL Design, Logic Verification & VLSI Systems"
        self.languages   = ["Verilog", "SystemVerilog", "C", "Python"]
        self.tools       = ["Xilinx Vivado", "Vitis", "GTKWave", "MATLAB", "LTspice"]
        self.protocols   = ["UART", "SPI", "I2C", "AXI4-Lite", "AXI4"]
        self.interests   = ["FPGA Design", "Digital Logic", "Embedded Systems", "VLSI"]
        self.status      = "Designing synthesizable hardware & verifying digital systems ⚡"

    def say_hi(self):
        print("Thanks for visiting! Let's build something in silicon 🔌")

me = DigitalSystemsEngineer()
me.say_hi()
```

---

## `cat skills`

<div align="center">

### ⚡ Hardware Description Languages

<p>
  <img src="https://img.shields.io/badge/Verilog_HDL-%2300599C?style=for-the-badge&logoColor=white" />
  <img src="https://img.shields.io/badge/SystemVerilog-%231572B6?style=for-the-badge&logoColor=white" />
  <img src="https://img.shields.io/badge/Logic_Design-FF6B6B?style=for-the-badge&logoColor=white" />
  <img src="https://img.shields.io/badge/FSM_Design-9b5de5?style=for-the-badge&logoColor=white" />
</p>

### 💻 Programming & Scripting

<p>
  <img src="https://img.shields.io/badge/C-%23A8B9CC?style=for-the-badge&logo=c&logoColor=black" />
  <img src="https://img.shields.io/badge/Python-3670A0?style=for-the-badge&logo=python&logoColor=ffdd54" />
</p>

### 🛠️ Tools & Engineering Software

<p>
  <img src="https://img.shields.io/badge/Xilinx_Vivado-FF6B6B?style=for-the-badge&logoColor=white" />
  <img src="https://img.shields.io/badge/Xilinx_Vitis-4D96FF?style=for-the-badge&logoColor=white" />
  <img src="https://img.shields.io/badge/GTKWave-2b2d3b?style=for-the-badge&logoColor=white" />
  <img src="https://img.shields.io/badge/MATLAB-ED1C24?style=for-the-badge&logo=mathworks&logoColor=white" />
  <img src="https://img.shields.io/badge/LTspice-808080?style=for-the-badge&logoColor=white" />
</p>

### 🌐 Protocols & Bus Interfaces

<p>
  <img src="https://img.shields.io/badge/UART-00b894?style=for-the-badge&logoColor=white" />
  <img src="https://img.shields.io/badge/SPI-0984e3?style=for-the-badge&logoColor=white" />
  <img src="https://img.shields.io/badge/I2C-6c5ce7?style=for-the-badge&logoColor=white" />
  <img src="https://img.shields.io/badge/AXI4--Lite-e17055?style=for-the-badge&logoColor=white" />
  <img src="https://img.shields.io/badge/AXI4-fdcb6e?style=for-the-badge&logoColor=black" />
  <img src="https://img.shields.io/badge/FIFO-a29bfe?style=for-the-badge&logoColor=white" />
</p>

</div>

---

### 🎯 Skill Proficiency

| Domain | Proficiency |
| :--- | :--- |
| RTL Design & Hardware Modeling | `████████████████████████░` 90% |
| Logic Verification & Testbench Design | `███████████████████████░░` 85% |
| Protocol Compliance (UART, SPI, AXI) | `████████████████████████░` 90% |
| Web Application Development (Flask) | `████████████████████░░░░░` 80% |

---

## 📂 Projects

<table>
<tr>
<td width="50%" valign="top">

### 🎛️ UART Communication Subsystem

> Synthesizable UART Tx/Rx with 16x oversampling, parameterized baud-rate generation, and timing-accurate async serial communication.

- FSM-driven protocol control with start/stop bit validation and mid-bit sampling to mitigate clock skew
- Self-checking verification testbench with automated loopback testing & GTKWave trace analysis

<p>
  <img src="https://img.shields.io/badge/Verilog_HDL-00599C?style=flat-square" />
  <img src="https://img.shields.io/badge/Vivado-FF6B6B?style=flat-square" />
  <img src="https://img.shields.io/badge/GTKWave-2b2d3b?style=flat-square&logoColor=white" />
</p>

</td>
<td width="50%" valign="top">

### 🔌 SPI Master Controller

> Parameterized SPI Master IP supporting all 4 SPI modes via runtime CPOL/CPHA config, programmable clock division & variable data-width registers.

- FSM-driven transaction control with SerDes shift registers and protocol-compliant clock generation
- Self-checking testbench with randomized stimulus generation and scoreboard-based validation

<p>
  <img src="https://img.shields.io/badge/Verilog_HDL-00599C?style=flat-square" />
  <img src="https://img.shields.io/badge/SystemVerilog-1572B6?style=flat-square" />
  <img src="https://img.shields.io/badge/Verification-9b5de5?style=flat-square" />
</p>

</td>
</tr>
</table>

---

## 📜 Certifications & Achievements

| 🏅 Certification | 🏛️ Issuer |
| :--- | :--- |
| Hardware Modeling Using Verilog | NPTEL — IIT Kharagpur |
| MATLAB Onramp | MathWorks |

---

## `./github --stats`

<div align="center">

<img src="https://github-readme-stats.vercel.app/api?username=rk9334183132-bit&show_icons=true&theme=tokyonight&count_private=true&hide_border=true&bg_color=0d1117&title_color=7aa2f7&icon_color=c792ea&text_color=a9b1d6" width="48%" />
<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=rk9334183132-bit&layout=compact&theme=tokyonight&hide_border=true&bg_color=0d1117&title_color=7aa2f7&text_color=a9b1d6" width="48%" />

<img src="https://github-readme-streak-stats.herokuapp.com?user=rk9334183132-bit&theme=tokyonight&hide_border=true&background=0d1117&ring=7aa2f7&fire=c792ea&currStreakLabel=a9b1d6" width="60%" />

<img src="https://github-readme-activity-graph.vercel.app/graph?username=rk9334183132-bit&theme=tokyo-night&hide_border=true&bg_color=0d1117&color=7aa2f7&line=c792ea&point=7aa2f7" width="98%" />

</div>

---

## `contact --info`

<div align="center">

**Let's connect and build something incredible ✨**

<p>
  <a href="https://linkedin.com/in/rakesh-kumar-b57127220">
    <img src="https://img.shields.io/badge/LinkedIn-Connect-%230077B5?style=for-the-badge&logo=linkedin&logoColor=white" />
  </a>
  <a href="mailto:your.email@gmail.com">
    <img src="https://img.shields.io/badge/Email-Reach%20Out-D14836?style=for-the-badge&logo=gmail&logoColor=white" />
  </a>
</p>

<img src="https://komarev.com/ghpvc/?username=rk9334183132-bit&label=PROFILE+VIEWS&color=7aa2f7&style=flat-square" />

</div>

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:24243e,50:302b63,100:0f0c29&height=120&section=footer" />
